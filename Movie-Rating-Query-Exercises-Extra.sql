...
-- 3. Return all reviewer names and movie names together in
--    a single list, alphabetized.
...
-- 8. Find the names of all reviewers who have
--    contributed three or more ratings. (As an extra challenge,
...
select distinct name
from Rating a join
  Rating b using(rID) join
  Rating c using(rID) join
  Reviewer using(rID)
...
-- 12. For each director, return the director's name together with the title(s)
